...
def main(config:str, cuda:int):
    ...
    for video in annotations["annotations"]:
        if annotations["annotations"][video]["scenario"] == scenario:
            ...
            for i in range(1, len(correct_key_dict_of_video) - 1):
                ...
                video_embedding = correct_key_dict_of_video[i]["video_embedding"]
                video_embedding_2 = correct_key_dict_of_video[i+1]["video_embedding"]

                video_embedding_analisys = torch.stack([video_embedding_1, video_embedding, video_embedding_2], dim=0)
                ...
                batch_size = torch.tensor(np.arange(3)).to(device)
                ...
                sequence_true = [0, 1, 2, 3, 4]
                sequence_false = [0, 3, 2, 1, 4]
                if rate(sequence_true, pred_adjacency_matrix) > rate(sequence_false, pred_adjacency_matrix) or pred_adjacency_matrix[2, 3] < pred_adjacency_matrix[2, 1]:
                    preds.append(1)
                else:
                    preds.append(0)
    ...
    with open(f"forecasting/{scenario}.txt", "w") as f:
