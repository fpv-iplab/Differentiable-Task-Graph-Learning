...
# This script runs train_with_gt_videos.py for all yaml files in the CaptainCook4D folder.
...
def main(more_seeds:bool):
    ...
    print("Running train_all_TGT-video.py")
    ...
    def run_training(yaml_file, device):
        ...
        if more_seeds:
            ...
            for seed in [42, 1337, 2024, 2025, 2026]:
                command = f"python train_with_gt_videos.py --config ../../configs/CaptainCook4D-TGT-video/{yaml_file} --seed {seed} --log --cuda {device}"
                os.system(command)
        else:
            command = f"python train_with_gt_videos.py --config ../../configs/CaptainCook4D-TGT-video/{yaml_file} --log --cuda {device}"
            os.system(command)
        print(f"Finished training for {yaml_file}")

    threads = []
    for yaml_file in os.listdir("../../configs/CaptainCook4D-TGT-video"):
