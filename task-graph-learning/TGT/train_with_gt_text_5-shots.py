...
@click.option("--max_length", type=int, default=5, help="Maximum length of the sequences.")
@click.option("--save", type=bool, default=False, is_flag=True, help="Save the model.")
def main(config:str, pre_trained:str, log:bool, seed:int, cuda:int, w:bool, project_name:str, entity:str, max_length:int, save:bool):
    ...
    # Output path
    output_path = cfg.OUTPUT_DIR + "-5-shots"
    ...
    for video in annotations_json:
        ...
        if max_length > 0:
            if len(train_sequences) == max_length:
                break
            train_sequences.append(sequences)
        elif sequences not in train_sequences:
            train_sequences.append(sequences)
    ...
    if max_length > 0:
        ...
        # Choose a random sequence to add
        while len(train_sequences) < max_length:
            random_sequence = random.choice(train_sequences)
            train_sequences.append(random_sequence)
    ...
    if save:
        torch.save(net.state_dict(), os.path.join(output_path, f"{activity_name}", f"model_{activity_name}_{cfg.SEED}.pth"))
