    ...
    def run_training(yaml_file, device):
        ...
        if more_seeds:
            ...
            for seed in [42, 1337, 2024, 2025, 2026]:
                command = f"python train_with_gt.py --config ../../configs/CaptainCook4D-TGT-text/{yaml_file} --seed {seed} --log --cuda {device}"
                os.system(command)
        else:
            command = f"python train_with_gt.py --config ../../configs/CaptainCook4D-TGT-text/{yaml_file} --log --cuda {device}"
            os.system(command)
